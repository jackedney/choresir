...
@pytest.mark.asyncio
async def test_yes_confirms_invite_and_welcomes_user(mock_db_module, db_client) -> None:
    ...
    # Step 3: Create pending invite (simulating admin invite flow)
    ...
    assert "Welcome to TestHouse" in result
    assert "membership is now active" in result


@pytest.mark.integration
@pytest.mark.asyncio
async def test_yes_case_insensitive_confirms_invite(mock_db_module, db_client) -> None:
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_non_yes_message_returns_instruction(mock_db_module, db_client) -> None:
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_no_pending_invite_returns_not_member_message(mock_db_module, db_client) -> None:
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_pending_invite_but_no_user_record_returns_error(mock_db_module, db_client) -> None:
