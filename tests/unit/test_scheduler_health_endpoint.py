...
def test_scheduler_health_endpoint_all_jobs_healthy(client: TestClient) -> None:
    ...
    async def mock_get_job_status(job_name: str) -> dict:
        """Mock get_job_status to return a healthy status."""
        return {
            "job_name": job_name,
    ...
    with patch("src.main.job_tracker") as mock_tracker:
        mock_tracker.get_job_status = mock_get_job_status
...
def test_scheduler_health_endpoint_degraded_with_failures(client: TestClient) -> None:
    ...
    async def mock_get_job_status(job_name: str) -> dict:
        """Mock get_job_status to return a status with failures."""
        return {
            "job_name": job_name,
            "last_success": "2024-01-01T00:00:00Z",
...
def test_scheduler_health_endpoint_critical_with_dlq(client: TestClient) -> None:
    ...
    async def mock_get_job_status(job_name: str) -> dict:
        """Mock get_job_status to return a status with critical failures."""
        return {
            "job_name": job_name,
            "last_success": None,
            "last_failure": "2024-01-01T01:00:00Z",
