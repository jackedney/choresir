...
class TestTaskService:
    async def _create_active_member(
        self, session, whatsapp_id="member@c.us"
    ):
        member = make_member(
            whatsapp_id=whatsapp_id, status=MemberStatus.ACTIVE
        )
        session.add(member)
    ...
    @pytest.mark.anyio
    async def test_claim_completion_peer_mode_stays_claimed(
        self, session
    ):
        member = await self._create_active_member(session)
        svc = TaskService(session)
    ...
    async def test_verify_completion(self, session):
        member = await self._create_active_member(session)
        verifier = await self._create_active_member(
            session, "verifier@c.us"
        )
        svc = TaskService(session)
        task = await svc.create_task(
            title="Verified task",
    ...
    async def test_self_verification_raises(self, session):
        ...
        await svc.claim_completion(task.id, member.id)
        with pytest.raises(
            AuthorizationError, match="Cannot verify your own"
        ):
    ...
    async def test_reassign(self, session):
        member = await self._create_active_member(session)
        new_member = await self._create_active_member(
            session, "new@c.us"
        )
        svc = TaskService(session)
        task = await svc.create_task(
