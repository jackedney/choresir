...
**Note:** Historical entries reference PocketBase, which was the original database backend. The project has since migrated to SQLite with aiosqlite (see 2026 migration commits).
...
## 2026-02-18 - [Public API Access Lockdown] [HISTORICAL - PocketBase]
...
**Status:** This vulnerability no longer applies after migration to SQLite, which doesn't have public API rules.

## 2026-02-19 - [Sensitive Data Exposure in Auxiliary Collections] [HISTORICAL - PocketBase]
...
**Vulnerability:** Several services (`chore_service`, `analytics_service`, `robin_hood_service`) constructed filter queries using f-strings with unsanitized user inputs (e.g., `f'assigned_to = "{user_id}"'`). This allows attackers to bypass filters using injection payloads like `" || true || "`.
...
**Current Status:** Still applies to SQLite implementation. Filter queries must use `sanitize_param()` and parameterized queries.

## 2026-02-06 - [Hardcoded Admin Credentials in Config] [HISTORICAL - PocketBase]
...
**Status:** This vulnerability no longer applies after migration to SQLite, which doesn't require admin credentials.
...
## 2026-03-09 - [SQL Injection via Subquery in Custom Filter Parser]
**Vulnerability:** The `_get_last_completion_date` function constructed a custom `filter_query` using an SQL subquery (`user_id IN (SELECT id FROM members WHERE phone = "...")`). The custom filter parser `db_client._parse_filter` doesn't support complex syntax like `IN (SELECT ...)`, leading to either parse errors when unescaped or potential bypasses/injections since the subquery bypasses typical DB parameters abstraction.
**Learning:** Mixing direct SQL constructs like `IN (SELECT ...)` with a custom high-level filter abstraction (`field = "value" && ...`) is a pattern that introduces both runtime parsing errors and severe SQL injection vulnerabilities (Bandit B608) if input isn't flawlessly sanitized.
**Prevention:** Break complex join requirements into sequential, simple queries using the supported filter syntax. Fetch dependencies (like user ID) in one query, validate, and then pass them explicitly to the next query using standard equality operators.
