...
## 2025-03-01 - Sentinel: [MEDIUM] Fix SQL Injection Vector in Scheduler Jobs
**Vulnerability:** A potential SQL injection vector existed in `src/modules/tasks/scheduler_jobs.py` (`_get_last_completion_date`) due to an invalid SQL subquery (`IN (SELECT ...)`) injected directly into a filter string.
**Learning:** The custom PocketBase-style filter parser in `src/core/db_client.py` does not support SQL subqueries. Using them bypasses the abstraction layer, triggers SQL injection security warnings (Bandit B608), and breaks the application logic.
**Prevention:** Always break down complex joins or subqueries into sequential, valid database queries using the supported `get_record`, `get_first_record`, or `list_records` methods instead of injecting custom SQL into the filter string.
