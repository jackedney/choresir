...
## 2024-05-18 - Admin Login Rate Limit Bypass via IP Spoofing
**Vulnerability:** Missing rate limiting on the `/admin/login/submit` endpoint allowed brute-forcing admin passwords. An initial implementation of rate limiting cleared the attempts dictionary when full (`_login_attempts.clear()`), which introduced a DoS vector and rate limit bypass by spoofing `X-Forwarded-For` IPs to instantly clear the history.
**Learning:** In-memory dictionaries used for rate limiting must be size-bounded safely. Relying on `.clear()` when full allows attackers to flood the system with fake IPs to reset their own IP's rate limit.
**Prevention:** Iteratively evict old items and if the dictionary size limit is reached, safely remove the oldest or expired entries instead of clearing the entire dictionary.
