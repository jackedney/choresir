...
## 2026-10-28 - [Rate Limiting Admin Login]
**Vulnerability:** The admin login endpoint `/admin/login/submit` was vulnerable to brute-force attacks and credential stuffing because it did not limit the number of authentication attempts per client.
**Learning:** In-memory rate limiting implementation requires care. It must safely parse proxy headers (`X-Forwarded-For`), while bounding memory usage. Furthermore, dictionary eviction strategies for shared state in asynchronous multi-threaded environments must avoid `RuntimeError: dictionary changed size during iteration` and `KeyError` race conditions by using strategies like copying keys and checking `.get()`.
**Prevention:** Implemented an in-memory IP-based rate limiter capping attempts to 5 per minute per IP, logging successful attempts to clear the history, and periodically evicting expired entries safely.
