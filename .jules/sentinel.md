...
## 2026-10-28 - [Rate Limiting Logic Bug on FastHTML Admin Login]
**Vulnerability:** FastHTML route handlers lack built-in rate limiting tools out of the box and manual implementation using `time.time()` may introduce application breaking `NameError` if imports are forgotten.
**Learning:** Manual rate limiting requires careful state management to avoid memory exhaustion (e.g. dictionary size capping) and precise testing including HTTP status checks to confirm redirection or exceptions.
**Prevention:** Ensured the rate limiting mechanism implemented tracks attempts correctly, resets when hitting boundaries, uses `secrets.compare_digest` for password verification, properly imports dependencies (`time`, `RateLimitExceededError`), and registers application level exception handlers correctly across the application and FastHTML instance.
