...
- **Basic swap logging**: The `is_swap` parameter exists in `LogChore` tool
...
See task file `003-complete-robin-hood-protocol.md` in `.context/` for completion roadmap.
...
We will implement a "Robin Hood Protocol" - a chore takeover system that allows any household member to complete another member's assigned chore with the following rules:
...
4. **Weekly Limits**: Each household member can participate in a maximum of 3 takeovers per week (either as taker or original assignee). This limit resets every Monday at 00:00 in the household's timezone.
...
Require the taker to give one of their chores to the original assignee.
...
- **Rejected**: Removes accountability for the original assignee and could enable dependency
...
- [ADR 001: Technology Stack](001-stack.md) - Defines the primary user interface (Twilio WhatsApp) through which Robin Hood Protocol is accessed
- [ADR 002: Agent Framework](002-agent-framework.md) - Agent design for interpreting takeover commands
- [ADR 003: Verification Protocol](003-verification.md) - Verification requirements that apply to takeover completions
- [ADR 008: Gamification](008-gamification.md) - Points system affected by takeover logic
