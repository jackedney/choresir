...
We chose to create **distinct database collections** (`personal_chores`, `personal_chore_logs`) rather than extending the existing `chores` and `logs` tables with a `is_personal` flag.
...
- Accountability partner sees only verification requests (not general household stats)

**Exception:** Accountability partners receive verification requests for the specific chores they're assigned to verify, but these are private 1-on-1 interactions, not broadcast to the household.
...
- Personal chores are PRIVATE to the user (no leaderboard, no household visibility)
- Verification requests go only to the accountability partner (if set)
...
We reused the **recurrence parsing logic** and **agent framework** from household chores.
...
- **No Historical Partner Changes:** If partner is changed, old logs still reference original partner
...
- Creates frustrating UX (user did the work but can't get credit)
